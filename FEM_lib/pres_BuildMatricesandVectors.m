function [S,f] = pres_BuildMatricesandVectors(myCFD)
%
...
for el_index = 1:length(elmat(:,1)) % for all internal elements
	Selem = pres_GenerateElementMatrix(el_index,myCFD); % Selem	
    for ind1 = 1:topology
        for ind2 = 1:topology
            S(elmat(el_index,ind1),elmat(el_index,ind2))	= S(elmat(el_index,ind1),elmat(el_index,ind2)) + Selem(ind1,ind2);
        end
    end
    felem = pres_GenerateElementVector(el_index,myCFD); % felem
    for ind1 = 1:topology
...
for el_index = 1:length(elmatbnd(:,1)) % for all boundary elements extension of mass matrix M and element vector f
	BMelem = pres_GenerateBoundaryElementMatrix(el_index,myCFD); % BMelem
	...
	bfelem = pres_GenerateBoundaryElementVector(el_index,myCFD); % bfelem   
    for ind1 = 1:topologybnd
